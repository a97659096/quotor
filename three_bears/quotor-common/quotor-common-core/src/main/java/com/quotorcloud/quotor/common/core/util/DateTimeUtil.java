...
public class DateTimeUtil {
     ...
     * @param dto
     */
    public static void setStartAndEndDate(Class<?> clazz, Object dto){
        try {
            String dateRange = (String) MethodUtil.getGetMethod(dto, "dateRange");
            if(!ComUtil.isEmpty(dateRange)){
                List<String> stringDate = getStringDate(dateRange);
                if(!ComUtil.isEmpty(stringDate)){
                    MethodUtil.setValue(dto,clazz,"start",String.class,stringDate.get(0));
                    MethodUtil.setValue(dto,clazz,"end",String.class,stringDate.get(1));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
     ...
     */
    public static List<String> getStringDate(String dateRange) {
        if(ComUtil.isEmpty(dateRange)){
            return null;
        }
        //分割成集合
    ...
    public static LocalDate stringToLocalDate(String date){
        return LocalDate.parse(date, DateTimeFormatter.ofPattern(FMT_yyyyMMdd));
    }
